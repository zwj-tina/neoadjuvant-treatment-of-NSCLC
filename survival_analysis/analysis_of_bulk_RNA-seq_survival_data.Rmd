...
fit.data <- OAK.meta.ICI %>% mutate(
  b.cell.signature=colSums(OAK.ICI.z.scored[b.cell.signature,]),
  treg.signature=colSums(OAK.ICI.z.scored[treg.signature,]),
  texp.signature=colSums(OAK.ICI.z.scored[texp.signature,]),
) %>% mutate(
  treg.texp=ifelse(treg.signature>=texp.signature, "treg>texp", "treg<texp"),
...
### tregtexp
```{r}
fit.cox <- coxph(Surv(fit.data$OS_MONTHS, fit.data$OS_CENSOR) ~ fit.data$treg.texp, data = fit.data)
summary(fit.cox)

fit <- survfit(Surv(fit.data$OS_MONTHS, fit.data$OS_CENSOR) ~ fit.data$treg.texp, data = fit.data)
print(fit)
...
ggsave("bulk_data_plots/OAK_dataset_treg_compared_with_texp.pdf", width = 8, height = 6)
```
